...
config_presets = {
    'Flow-3D-img224-LowRes':  dnnlib.EasyDict(duration=2048<<20, batch=2048, channels=64,  lr=0.0170, decay=35000, dropout=0.00, P_mean=-0.4, P_std=1.0),
   }

#----------------------------------------------------------------------------
# Setup arguments for training.training_loop.training_loop().

def setup_training_config(preset='Flow-3D-img224-LowRes', **opts):
    ...
    c.network_kwargs = dnnlib.EasyDict(class_name='training.networks_edm2.Flow3D_conditional', model_channels=opts.channels, dropout=opts.dropout)
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.training_loop.FlowLoss3D_conditional', P_mean=opts.P_mean, P_std=opts.P_std)
...
@click.option('--preset',           help='Configuration preset', metavar='STR',                 type=str, default='Flow-3D-img224-LowRes', show_default=True)
