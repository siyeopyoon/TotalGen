...
@persistence.persistent_class
class EDM2Loss3D_conditional:
    def __init__(self, P_mean=-0.4, P_std=1.0, sigma_data=0.5):
        self.P_mean = P_mean
        self.P_std = P_std
        self.sigma_data = sigma_data

    def __call__(self, net, images, conditions, labels=None):
        rnd_normal = torch.randn([images.shape[0], 1, 1, 1, 1], device=images.device)
        sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        weight = (sigma ** 2 + self.sigma_data ** 2) / (sigma * self.sigma_data) ** 2
        noise = torch.randn_like(images) * sigma
        denoised, logvar = net(images + noise, sigma, conditions, labels, return_logvar=True)
        loss = (weight / logvar.exp()) * ((denoised - images) ** 2) + logvar
        return loss
...
class FlowLoss3D_conditional:
    ...
    def __call__(self, net, images, conditions, labels=None):
        ...
        path_sample = self.path.sample(t=time, x_0=noise, x_1=images)

        x_t = path_sample.x_t
        u_t = path_sample.dx_t
        time=time.view(images.shape[0], 1, 1, 1, 1).to(device=images.device)

        est = net(x_t, time, conditions, labels, return_logvar=False)
        loss = (est - u_t) ** 2
        return loss
...
def training_loop(
    ...
    encoder_kwargs      = dict(class_name='training.encoders.StandardEncoder'),
    data_loader_kwargs  = dict(class_name='torch.utils.data.DataLoader', pin_memory=True, num_workers=2, prefetch_factor=2),
    network_kwargs      = dict(class_name='training.networks_edm2.Flow3D_conditional'),
    loss_kwargs         = dict(class_name='training.training_loop.FlowLoss3D_conditional'),
    ...
    ref_image, ref_cond, ref_label = dataset_obj[0]
    ...
    if ref_label is not None:
        interface_kwargs = dict(img_resolution=ref_image.shape[-1], img_channels=ref_image.shape[1], cond_channels=ref_cond.shape[0],label_dim=ref_label)
    else:
        interface_kwargs = dict(img_resolution=ref_image.shape[-1], img_channels=ref_image.shape[1], cond_channels=ref_cond.shape[0], label_dim=0)
    ...
    if dist.get_rank() == 0:
        misc.print_module_summary(net, [
            torch.zeros([batch_gpu, net.img_channels,ref_image.shape[-3],ref_image.shape[-2],ref_image.shape[-1]], device=device), #image
            torch.ones ([batch_gpu], device=device),# noise-sigma
            torch.zeros([batch_gpu, net.cond_channels,ref_image.shape[-3],ref_image.shape[-2],ref_image.shape[-1]], device=device), #condition
            torch.zeros([batch_gpu, net.label_dim], device=device), #label
        ], max_nesting=2)
    ...
    while True:
        ...
        if checkpoint_nimg is not None and (done or state.cur_nimg % checkpoint_nimg == 0) and state.cur_nimg != start_nimg:
            ...
            dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs)# Main training loop.
            dataset_sampler = misc.InfiniteSampler(dataset=dataset_obj, rank=dist.get_rank(), num_replicas=dist.get_world_size(), seed=seed, start_idx=state.cur_nimg)
            dataset_iterator = iter(dnnlib.util.construct_class_by_name(dataset=dataset_obj, sampler=dataset_sampler, batch_size=batch_gpu, **data_loader_kwargs))
        ...
        for round_idx in range(num_accumulation_rounds):
            ...
            if device != "cpu":
                with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                    images, conditions, labels = next(dataset_iterator)
                    images = encoder.encode_latents(images.to(device))
                    conditions = conditions.to(device)
                    loss = loss_fn(net=ddp, images=images, conditions= conditions, labels=labels.to(device))
        ...
        training_stats.report('Gain',  net.unet3D.out_gain.data.item())
