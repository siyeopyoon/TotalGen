...
config_presets = {
    'edm2-3D-img224-LowRes':  dnnlib.EasyDict(duration=2048<<20, batch=2048, channels=64,  lr=0.0170, decay=35000, dropout=0.00, P_mean=-0.4, P_std=1.0),
   }

#----------------------------------------------------------------------------
# Setup arguments for training.training_loop.training_loop().

def setup_training_config(preset='edm2-3D-img224-LowRes', **opts):
    ...
    # Dataset.
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.Dataset_train_low_PETCT', 
                                       path=opts.data, use_labels=opts.get('cond', True),dry=True)
    try:
    ...
    #actual datasets
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.Dataset_train_low_PETCT', 
                                       path=opts.data, use_labels=opts.get('cond', True),dry=False)
    ...
    if dataset_channels == 3:
        ...
        c.encoder_kwargs = dnnlib.EasyDict(class_name='training.encoders.StandardEncoder')
    ...
    c.network_kwargs = dnnlib.EasyDict(class_name='training.networks_edm2.Precond3D_conditional', model_channels=opts.channels, dropout=opts.dropout)
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.training_loop.EDM2Loss3D_conditional', P_mean=opts.P_mean, P_std=opts.P_std)
...
# Hardcoded paths for the dataset and results directory.
...
data_dir =f"{external}/2_Datasets/20_TCIA_FDG-PET-CT-Lesions/Dataprocess"
results_dir = f"{external}/2_Datasets/20_TCIA_FDG-PET-CT-Lesions/Trained"
...
@click.option('--preset',           help='Configuration preset', metavar='STR',                 type=str, default='edm2-3D-img224-LowRes', show_default=True)
...
# Performance-related options.
@click.option('--batch-gpu',        help='Limit batch size per GPU', metavar='NIMG',            type=parse_nimg, default=8, show_default=True)
...
# I/O-related options.
@click.option('--status',           help='Interval of status prints', metavar='NIMG',           type=parse_nimg, default='4Ki', show_default=True)
@click.option('--snapshot',         help='Interval of network snapshots', metavar='NIMG',       type=parse_nimg, default='64Ki', show_default=True)
@click.option('--checkpoint',       help='Interval of training checkpoints', metavar='NIMG',    type=parse_nimg, default='64Ki', show_default=True)
...
def cmdline(outdir, dry_run, **opts):
    ...
    outdir=os.path.join(outdir, opts["preset"])
    if use_gpu:
        dist.init()
