...
def flow_sampler_conditional(
    ...
    #             t0, t1 = time_grid[i], time_grid[i+1]
    #             dt = t1 - t0
    #             x = x + dt * ode_func(t0, x)
    #         return x
    ...
    #             k1 = ode_func(t0, x)
    #             k2 = ode_func(t0 + dt / 2, x + dt / 2 * k1)
    #             k3 = ode_func(t0 + dt / 2, x + dt / 2 * k2)
    #             k4 = ode_func(t1, x + dt * k3)

    #             x = x + dt / 6 * (k1 + 2 * k2 + 2 * k3 + k4)

    #         return x
    #     elif "ab2":
    #          # first step via Euler to get f_prev
    #         t0, t1 = time_grid[0], time_grid[1]
    #         dt = t1 - t0
    #         f_prev = ode_func(t0, x)
    #         x = x + dt * f_prev
    ...
    #             t0, t1 = time_grid[i], time_grid[i+1]
    #             dt = t1 - t0
    #             f_curr = ode_func(t0, x)
    #             # 2nd-order Adams–Bashforth
    #             x = x + dt * (3/2 * f_curr - 1/2 * f_prev)
    #             f_prev = f_curr
    #         return x
...
def generate_cascade(
    ...
    **sampler_kwargs,                           # Additional arguments for the sampler function.
    ...
    torch.manual_seed(0) # fixed seed for consistency
    noise = torch.randn([1, net.img_channels, 56, 56, 96], device=device)
    ...
    for Weight in Weight_Range:
        ...
        for cidx in range (net.img_channels):
            ...
            affine[3,3]=1.0
            new_image = nibabel.Nifti1Image(output, affine=affine)
            nibabel.save(new_image, f'{outdir}images_{modality}_{data["height"]}_{data["weight"]}_{data["sex"]}_{data["age"]}.nii.gz')
        ...
        conditions = torch.from_numpy(demographics).unsqueeze(0).to(device)
        #print (conditions.shape)
        lowres = torch.from_numpy(lowres).unsqueeze(0).to(device)
        #print (lowres.shape)
        conditions=torch.cat ([lowres,conditions],axis=1)
        ...
        while current_start < D:
            ...
            noise_partial = SR_noise[..., extended_start:extended_end].to(device)
            conditions_partial = conditions[..., extended_start:extended_end].to(device)

            
            
                
            out_extended = dnnlib.util.call_func_by_name(func_name=flow_sampler_conditional, net=srnet, noise=noise_partial, outfolder=outdir,
...
config_presets = {
    'cascade-Flow-3D-img224':  dnnlib.EasyDict(net=f'{model_root}/Flow_LowRes_network.pkl',
                                             srnet=f'{model_root}/Flow_SR_network.pkl'),      # fid = 3.53=
    }



@click.command()
@click.option('--preset',                   help='Configuration preset', metavar='STR',                             type=str,  default='cascade-Flow-3D-img224')
